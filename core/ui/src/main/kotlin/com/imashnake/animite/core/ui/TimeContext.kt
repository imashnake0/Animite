...
data class TimeContext(
    ...
    companion object {
        ...
        fun calculateFromTime(
            ...
            val dayProgress = time.toSecondOfDay() / LocalTime(23, 59).toSecondOfDay().toFloat()
            return TimeContext(
 ...
 */
@Composable
fun produceTimeContext(
    clock: Clock = Clock.System,
    timeZone: TimeZone = TimeZone.currentSystemDefault()
): State<TimeContext> = produceState(
    TimeContext.calculateFromNow(clock, timeZone)
) {
    while (this.coroutineContext.isActive) {
        delay(1.minutes)
        value = TimeContext.calculateFromNow(clock, timeZone)
    }
}
